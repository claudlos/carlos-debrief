#!/usr/bin/env python3
"""Generate a Hermes-judged Top 500 page (scored out of 1000) from historical debriefs."""
...
def build_top500(items: list[Card], ratings: dict[str, dict[str, Any]], limit: int) -> list[dict[str, Any]]:
...
def render_html(data: dict[str, Any]) -> str:
    ...
    source_options = "\n".join(
        ...
        for source in data["stats"]["top500_by_source"]
    )
...
<div class="wrap">
  ...
  <footer>
    Generated by <code>scripts/generate_top500.py</code>. Data artifact: <a href="top500.json" style="color:var(--text-muted);">top500.json</a>.
  </footer>
</div>
<script type="application/json" id="top500-data">{items_json}</script>
<script>
(function () {{
...
def build_output_data(
    ...
    return {
        ...
        "stats": {
            **stats,
            "top500_by_source": count_by(top_items, "source"),
            "top500_by_type": count_by(top_items, "type"),
        },
        "items": top_items,
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cache", default=".top500-ratings-cache.json", type=Path)
    parser.add_argument("--output-json", default="top500.json", type=Path)
    parser.add_argument("--output-html", default="top-500.html", type=Path)
    parser.add_argument("--dry-run", action="store_true", help="Parse and print stats without calling Hermes or writing outputs.")
    parser.add_argument("--render-only", action="store_true", help="Regenerate HTML from an existing top500 JSON file.")
...
def main() -> int:
    ...
    top_items = build_top500(items, ratings, args.limit)
